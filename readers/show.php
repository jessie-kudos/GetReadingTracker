<?php
	...
	$readerId = htmlspecialchars($_GET["id"]);
	$_SESSION['error'] = '';
	$_SESSION['info'] = '';
	...
	$sql = "SELECT name, username FROM readers WHERE id = '$readerId'";

	if (!$result = $mysqli->query($sql)) {
		echo "Error finding reader.";
	}
...
<html lang="en">
	<head>
		...
		<link rel="stylesheet" type="text/css" href="/assets/stylesheets/jtable.min.css">
	  ...
	  <div class="grid-x">
			...
			<div class="cell top-bar-cell">
				...
				<div class="top-bar user-show-top-bar" id="responsive-menu">
					...
					<div class="top-bar-right">
						<ul class="menu">
							<li><a href="/users/logout.php" title="Log Out">Log Out</a></li>
						</ul>
					</div>
				</div>
			</div>	
			...
			<div class="grid-x">
				...
				<div class="small-9 cell">
					<div class="stats-list">
						<?php 
							$sql1 = "SELECT IFNULL(SUM(minutes),0) AS weeklyminutes FROM readingsessions WHERE (WEEK(sessiondate) = WEEK(CURDATE())) AND readerid = '$readerId'";
							$result1 = $mysqli->query($sql1);
							$row1 = $result1->fetch_assoc();
							$weeklyminutes = $row1['weeklyminutes'];

							$sql2 = "SELECT IFNULL(SUM(finished),0) AS weeklybooks FROM readingsessions WHERE (WEEK(sessiondate) = WEEK(CURDATE())) AND readerid = '$readerId'";
							$result2 = $mysqli->query($sql2);
							$row2 = $result2->fetch_assoc();
							$weeklybooks = $row2['weeklybooks'];

							$sql3 = "SELECT IFNULL(SUM(minutes),0) AS totalminutes FROM readingsessions WHERE readerid = '$readerId'";
							$result3 = $mysqli->query($sql3);
							$row3 = $result3->fetch_assoc();
							$totalminutes = $row3['totalminutes'];

							$sql4 = "SELECT IFNULL(SUM(finished),0) AS totalbooks FROM readingsessions WHERE readerid = '$readerId'";
		...
		<div class="tabs-content" data-tabs-content="readers-tabs">			
			<div class="tabs-panel is-active" id="panel1">
				<div class="grid-x">
					<div class="cell">
						<form class="add-reading-session-form" action="/reading_sessions/create.php" method="post">
				...
				<div class="grid-x">
					<div class="cell">
						...
						<div class="bar-graph-callout">
							...
							<?php
								$sql6 = "SELECT IFNULL(SUM(minutes),0) AS sundayminutes FROM readingsessions WHERE (WEEK(sessiondate) = WEEK(CURDATE())) AND WEEKDAY(sessiondate) = '6' AND readerid = '$readerId'";
								$result6 = $mysqli->query($sql6);
								$row6 = $result6->fetch_assoc();
								$sundayminutes = $row6['sundayminutes'];

								$sql7 = "SELECT IFNULL(SUM(minutes),0) AS mondayminutes FROM readingsessions WHERE (WEEK(sessiondate) = WEEK(CURDATE())) AND WEEKDAY(sessiondate) = '0' AND readerid = '$readerId'";
								$result7 = $mysqli->query($sql7);
								$row7 = $result7->fetch_assoc();
								$mondayminutes = $row7['mondayminutes'];

								$sql8 = "SELECT IFNULL(SUM(minutes),0) AS tuesdayminutes FROM readingsessions WHERE (WEEK(sessiondate) = WEEK(CURDATE())) AND WEEKDAY(sessiondate) = '1' AND readerid = '$readerId'";
								$result8 = $mysqli->query($sql8);
								$row8 = $result8->fetch_assoc();
								$tuesdayminutes = $row8['tuesdayminutes'];

								$sql9 = "SELECT IFNULL(SUM(minutes),0) AS wednesdayminutes FROM readingsessions WHERE (WEEK(sessiondate) = WEEK(CURDATE())) AND WEEKDAY(sessiondate) = '2' AND readerid = '$readerId'";
								$result9 = $mysqli->query($sql9);
								$row9 = $result9->fetch_assoc();
								$wednesdayminutes = $row9['wednesdayminutes'];

								$sql10 = "SELECT IFNULL(SUM(minutes),0) AS thursdayminutes FROM readingsessions WHERE (WEEK(sessiondate) = WEEK(CURDATE())) AND WEEKDAY(sessiondate) = '3' AND readerid = '$readerId'";
								$result10 = $mysqli->query($sql10);
								$row10 = $result10->fetch_assoc();
								$thursdayminutes = $row10['thursdayminutes'];

								$sql11 = "SELECT IFNULL(SUM(minutes),0) AS fridayminutes FROM readingsessions WHERE (WEEK(sessiondate) = WEEK(CURDATE())) AND WEEKDAY(sessiondate) = '4' AND readerid = '$readerId'";
								$result11 = $mysqli->query($sql11);
								$row11 = $result11->fetch_assoc();
								$fridayminutes = $row11['fridayminutes'];

								$sql12 = "SELECT IFNULL(SUM(minutes),0) AS saturdayminutes FROM readingsessions WHERE (WEEK(sessiondate) = WEEK(CURDATE())) AND WEEKDAY(sessiondate) = '5' AND readerid = '$readerId'";
								...
								<div class="grid-container">
									<div class="grid-x grid-padding-x">
										<div class="small-10 small-offset-1 cell">
											...
											<p><a href="/readers/show.php?id=<?php echo $readerId ?>" class="button expanded">Send</a></p>
										</div>
									</div>
								</div>
							</form>
		...
		<script src="/assets/javascripts/jquery.jtable.min.js"></script>
		<script src="/assets/javascripts/jquery.dynatable.js"></script>
